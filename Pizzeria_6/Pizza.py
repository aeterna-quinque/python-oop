...
from Pizzeria_6.OutOfRangeError import OutOfRangeError
...
class Pizza(ABC):
    ...
    async def change_ingredients(self):
        """
        ...
        keys_list = list(self.__ingredients.keys())
        for i in range(0, len(keys_list)):
            self.__ingredients[keys_list[i]] = await self.ingredients_interface(first_call, keys_list[i])
            first_call = False

    @ingredients_decorator
    async def ingredients_interface(self, first_call, ingredient):
        """
